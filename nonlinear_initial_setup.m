function [f, u_real, divide_matrix, u_real_boundary] = nonlinear_initial_setup(case_no,N,M,delta_M,delta_N,A,phi_bounded,c,a,b)
...
switch case_no
    case 1
        ...
        f=(-5.0).*(pi*pi).*(sin(pi.*A.*(cosh(delta_M/2+z1.*delta_M).*cos(z2.*delta_N))).*sin(2.*pi.*A.*(sinh(delta_M/2+z1.*delta_M).*sin(z2.*delta_N)))).*(denominator)-c*sinh(u_real');
        f=f';
    ...
    otherwise
        ...
        denominator=A.*A.*((sinh(delta_M/2+z1.*delta_M)).^2+(sin(z2.*delta_N)).^2);
        f=4*(denominator)./(a^2+b^2)-c*sinh(u_real');
        f=f';
        %sinh(u_real');
